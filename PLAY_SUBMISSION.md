...
| Item | Status | Notes |
|---|---|---|
| Account deletion — in-app | **Done, pending one manual step** | `DELETE /api/account` is implemented and wired to the account sheet. It deletes Firestore data (`devices`, `courses`, `progress`, root `users/{uid}` doc) then the Firebase Auth account via the Identity Platform REST API. **Caveat:** the service account's OAuth token is scoped to `identitytoolkit`, which requires an IAM role (e.g. "Firebase Authentication Admin") to actually be granted to that service account in Google Cloud IAM. If that grant hasn't been done yet, the Firestore-delete step will succeed but the Auth-account-delete step will fail (`ACCOUNT_DELETE_ERROR ... stage=auth`), leaving the login shell behind. **User action needed:** confirm the IAM role is granted; if not, grant it, then do one end-to-end test deletion. |
| Account deletion — web form | **Done, live** | `docs/delete-account.html` (linked from `privacy.html`), covers the no-app-access case, 4 languages, 30-day processing commitment. |
