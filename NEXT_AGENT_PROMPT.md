...
`py_compile`/`git diff --check` чистые. Flutter не менялся.

**Исправление статуса (внесено 20 июля 2026, при закрытии CR-17
ниже): фраза выше про uncommitted была верна только в момент записи.**
Эти изменения были закоммичены и запушены в ТОТ ЖЕ день: backend `b9345b2`
→ `phase3-2-promptfoo-gate`, Flutter-документация `8e0db1f` →
`phase5-account-deletion`. К моменту чтения этого файла оба коммита уже в
истории веток — не путать с «изменения ещё не сохранены».
...
### CR-17 write-once оказался неполным — исправлено после независимой перепроверки, 20 июля 2026

**Важно: write-once в `b9345b2` НЕ закрывал CR-17.** Независимая
перепроверка того же дня нашла: (1) `_cache_get()`→`_cache_set()` неатомарны
— два параллельных запроса к отсутствующему ключу оба видят MISS и оба
пишут, воспроизведено детерминированно; (2) first-writer poisoning остаётся
— любой authenticated пользователь (free или оплативший premium — оплата
ничего не доказывает) мог быть первым писателем ещё не существующего
shared-ключа. `SET NX` закрыл бы только (1), не (2).

**Fix переписан:** `POST /api/cache` больше никогда не создаёт новый
shared-cache ключ — единственная Redis-операция это чтение (`_cache_probe`);
`_cache_set` удалена из `main.py` полностью. Совпадающее значение → `200`
идемпотентно; отличающееся → `409 conflict`; ключ отсутствует → `403`,
создание отклонено. Upstash-ошибка → `503`, никогда мнимый success. Ключ
теперь строго валидируется (legacy 64-hex или `vN[.vM]|type|hash`).
Malformed non-object JSON (`[]`, `"str"`, `42`, `null`) больше не даёт 500
на `/api/cache`, `/api/device`, `/api/device/force`, и попутно на
`/api/parse`/`/api/tts` (тот же класс бага, вне исходного списка из трёх
эндпоинтов).

Признанный побочный эффект: `group`/`discover`-кэш (раньше писавшийся любым
клиентом любого тарифа, без premium-gate) теперь не может создать никто —
только `tools/inject_curated.py` вручную и только `doc`-ключи. Реальная
cost/latency регрессия для незнакомых документов, задокументированная как
осознанный компромисс, не забытая деталь. Полное обоснование —
`exam-trainer-api/PRODUCT_PLAN.md`, раздел «CR-17 доработан: write-once
оказался недостаточным».

Три субагента: threat-model аудит всех callers (Sonnet, подтвердил обратную
совместимость с Flutter), проектирование concurrency-regression тестов
(Sonnet, `tests/test_cache_concurrency_regression.py` с доказательством, что
harness ловит именно старый баг), независимый скептический финальный review
(Opus, подтвердил все 6 пунктов CLOSED, сам прогнал тесты, не нашёл обхода).
Backend gate: 244/244 (было 229), concurrency-тесты стабильны на 5
повторных прогонах, `py_compile`/`git diff --check` чистые. Flutter не
менялся.
...
### Handoff: Web Creator MVP — 21 июля 2026

Новый отдельный репозиторий: `/home/igor/project/exam_trainer_web_creator`
(локальный `main`, remote пока не назначен). Это не полный web-порт Android, а
узкий authoring companion: Firebase Auth → детерминированный `Lesen Teil 2`
editor → preview → существующий authenticated `POST /api/courses` → приватный
`ParsedCourse schema_version=1`. Gemini/PDF/shared-cache не используются.

Архитектурное решение подтверждено CodeGraph-аудитом двух существующих repo,
двумя Codex-субагентами и отдельным Claude Code CLI review с тремя внутренними
агентами/Firebase skills: не добавлять прямой Firestore SDK/write path. Backend
уже является единственной границей UID, revision CAS, tombstones и размера.

Текущее подтверждённое состояние:

- Web `flutter analyze` чист; unit/widget suite и fake end-to-end flow зелёные;
- web release bundle с dummy public Firebase defines собирается;
...
#### Дополнение: однополевый детерминированный импорт — 21 июля 2026

Web Creator больше не требует вручную раскладывать задание по четырём полям:
полный блок одного `Lesen Teil 2` разбирается локально в редактируемый черновик,
после чего проходит прежний preview/publish flow. Поддержаны natural PDF paste
для единственной пары 6–7 или 8–9 и строгие секции `[TITEL]`, `[TEXT]`,
`[FRAGEN]`, `[ANTWORTEN]`. Неоднозначность, неизвестный хвост, неполные маркеры
и ввод свыше 750 КиБ отклоняются вместо угадывания. Gemini/backend/schema v1
не менялись; повторный разбор не затирает ручные правки без подтверждения.
Web gates: analyze clean, 27/27 tests, Chrome 3/3, coverage 82.03%, release build.
