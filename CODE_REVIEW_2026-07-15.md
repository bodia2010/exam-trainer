...
| CR-17 | P1 / высокий, найден 20 июля 2026, первый фикс (commit `b9345b2`) оказался неполным, закрыт после независимой перепроверки в тот же день | Backend cache | Любой authenticated user мог перезаписать/first-write произвольный cache-ключ, включая curated `doc` |
| CR-18 | P1 / средний, найден 20 июля 2026, закрыт | Backend Firestore path | `deviceId` не валидировался перед конкатенацией в Firestore REST путь |
...
### Android SAF PDF picker smoke — 18 июля 2026

Закрыт отдельный тестовый пробел на границе Flutter и системного Android
DocumentsUI. Добавлен opt-in runner `tool/run_android_saf_picker_smoke.sh`,
который помещает fixture с ожидаемой `%PDF-` сигнатурой в отдельную папку Downloads и
запускает integration flavor. Тест использует настоящий `file_picker 10.3.10`,
реальный `PdfFileValidator` и проверяет, что SAF вернул читаемый путь с
корректными именем, размером и PDF-сигнатурой. После выбора подставляется
offline fake import service, поэтому production backend, Firebase Auth и
пользовательское хранилище курсов не затрагиваются.

Выбор файла оставлен одним явным действием оператора: DocumentsUI работает в
другом Android-процессе, а его OEM/locale UI нестабилен для координатной ADB-
автоматизации. Runner удаляет только созданную fixture-папку и integration
package и проверяет сохранность ранее установленного production package.
Invalid-signature и oversized случаи остаются в детерминированных host-тестах;
это даёт лучшее покрытие без хрупкого device gate.

Проверка пройдена 1/1 на Samsung SM-G985F (`192.168.1.42:42673`): DocumentsUI
...
### Premium full-PDF gate production smoke — 19 июля 2026

Тестовое устройство: Samsung SM-S938B (USB, RFCY51N8PEK), production APK
versionCode 10. Premium badge подтверждён в UI, библиотека курсов на старте
пустая.

Исходный 207-страничный flagship PDF (SHA-256
`53634b0c2c85cb2d6b9d5efabcf54a9a344ce5a7082e7a3b4cd1d0a5926149e9`) дал
production cache-hit и полный курс из 12 типов/142 items — ожидаемый путь для
уже известного документа.

Чтобы проверить путь для неизвестного документа под Premium-тарифом, к тому же
файлу временно добавлена нейтральная marker-страница: 208 страниц, SHA-256
...
версия прошла полный discovery+parse путь, а не отдалась из кэша.
...
90709, оценка стоимости $0.47198. Итоговая оценка — $0.90463. Во время импорта
произошёл один HTTP 502 на `/api/parse`; клиентский retry восстановился, импорт
успешно завершился примерно за 5 минут.
...
**Вывод:** structural Premium E2E — PASS (discovery/parse/tariff/502-retry/
persist через force-stop отработали корректно). Semantic correctness — NOT
...
После обновления wireless-debug порта тот же SM-G985F прошёл оба device-теста:
PDF flow 1/1 и Sprachbausteine accessibility при 200% text scale 1/1.
Integration package удалён, production package versionCode 11 и два локальных
курса сохранены. Обычный cold launch завершил preload и показал Home. Профиль
подтвердил Free account, а открытый полный cached course показал по одной
...
Следующий незакрытый пункт после v38 — P1 Этап 2 item 6 «Провести отдельный
backend security review», ни разу не отмечен выполненным ни в одном более
позднем handoff. Полный код-ревью `exam-trainer-api/main.py`,
`firebase_auth.py`, `firestore_client.py`. Firebase ID token verification
(RS256 через JWKS, audience/issuer проверены) и UID-изоляция (uid всегда из
верифицированного токена, никогда из тела запроса) — в порядке. Firestore
security rules не применимы: Flutter не использует `cloud_firestore`
напрямую, весь доступ — через backend с service-account REST.

Найдены и исправлены два реальных backend-дефекта (детали и rationale — в
`exam-trainer-api/PRODUCT_PLAN.md`, раздел «Первый проход backend security
review»):

- **CR-17 (новый) — cache poisoning в `POST /api/cache`.** Любой
  аутентифицированный пользователь (free тоже) мог безусловно перезаписать
  ЛЮБОЙ существующий cache-ключ, включая единственный вручную выверенный
  `v30.v38|doc|...`, на который опирается почти вся аудитория приложения —
  без всякой проверки связи между присланными `hash` и `value`. Теперь
  write-once (перезапись только байт-в-байт идентичным значением) плюс cap
  4 MiB на размер value; курируемая публикация (`inject_curated.py`) не
  затронута — она обращается к Upstash напрямую, не через этот роут.
- **CR-18 (новый) — непровалидированный `deviceId`.** `/api/device` и
  `/api/device/force` конкатенировали `deviceId` прямо в Firestore REST путь
  без проверки формата, в отличие от `course_id`, для которого точно такая
  же защита уже существовала по этой причине. Реальный `deviceId` — всегда
  UUID (`DeviceService.getDeviceId`), так что добавленная валидация не
  меняет поведение ни одного легитимного клиента.

Backend gate: 229/229 (было 217; +12 новых regression-тестов в
`tests/test_cache_endpoint_contract.py` и
`tests/test_device_endpoint_contract.py` — оба эндпойнта раньше не имели ни
одного теста), `py_compile` и `git diff --check` чистые. Flutter-код не
менялся (проблемы и фикс — только backend), Flutter gate в этой сессии не
перезапускался. Ничего не задеплоено — как и весь остальной backend-код,
требует отдельного подтверждения перед `vercel deploy`.

**Это не полный security review**, только первый проход одним агентом за
одну сессию: НЕ проверялись глубоко rate-limit устойчивость к
распределённому/множественному-аккаунту злоупотреблению за пределами уже
реализованной Фазы 0 (`_RATE_LIMIT_PER_HOUR`), `tts.py`, potential prompt
injection через содержимое PDF в Gemini-промпты (`prompts.py`), Upstash/
Gemini API-ключ rotation/exposure practices, устойчивость
`response_schemas.py`/`span_resolution.py` к откровенно враждебному
markdown. Пункт P1 «backend security review» остаётся частично открытым;
следующий агент должен продолжить с этого списка, а не считать пункт
закрытым.

### CR-17 доработан после независимой перепроверки — 20 июля 2026

**`b9345b2` (описанный выше) закрывал CR-17 преждевременно.** Независимая
перепроверка commit `b9345b2` в тот же день нашла: write-once реализация
(раздельные `_cache_get()` → `_cache_set()`) неатомарна — два параллельных
запроса к одному отсутствующему ключу оба видят MISS и оба пишут
(воспроизведено детерминированно); и first-writer poisoning остаётся — ЛЮБОЙ
authenticated пользователь (free или premium — оплата premium ничего не
доказывает) мог быть первым, кто создаёт ещё отсутствующий shared-ключ
произвольным содержимым. Атомарный `SET NX` закрыл бы только первую
проблему. Также найдены: cache key без валидации формата, malformed
non-object JSON (`[]`, `"str"`, `42`, `null`) вызывал сырой 500 на трёх
эндпоинтах, differing-value conflict маскировался под `200 {"ok": true}`,
ошибка Upstash не отличалась от «ключ подтверждённо отсутствует».

Оригинальный вывод CR-17 выше (замечание #1 в этом разделе) не удалён и
остаётся верным по существу находки — недостаточной оказалась именно
write-once реализация первого фикса, не сама находка проблемы.

**Fix переписан полностью:** `POST /api/cache` теперь никогда не создаёт
новый shared-cache ключ — единственная Redis-операция handler'а это чтение
(`_cache_probe`); `_cache_set` удалена из `main.py` целиком. Три исхода:
существующее значение совпадает → `200` идемпотентно без записи; отличается
→ `409 {"ok": false, "conflict": true}`; ключ отсутствует → `403`, создание
отклонено (не гонка за право создать — создавать нельзя вообще). Redis
недоступен/вернул неожиданное → `503`, никогда не мнимый success.

Это структурно устраняет ОБЕ проблемы одним решением: без вызова записи нет
ни гонки, ни «первый писатель побеждает». Публикация нового shared-контента
остаётся возможна только через `tools/inject_curated.py` (тот же
env `UPSTASH_REDIS_REST_URL`/`_TOKEN`, не отдельный секрет — отдельный
вручную запускаемый CLI-путь в обход Flask endpoint и Firebase auth, с
human-reviewed checklist). Явный признанный побочный эффект: `group`/
`discover`-кэш (раньше писавшийся любым клиентом любого тарифа при живом
импорте, без premium-gate) теперь не может создать никто — реальная
cost/latency регрессия для незнакомых документов, не только `doc`. Полное
обоснование, включая почему это правильный компромисс и рекомендованный
несделанный follow-up (доверенная server-side запись из `/api/parse`) — в
`exam-trainer-api/PRODUCT_PLAN.md`, раздел «CR-17 доработан: write-once
оказался недостаточным».

Реализация: главный агент спроектировал fix и написал основной код;
subagent (Sonnet) независимо проаудировал всех callers `/api/cache` в обоих
репозиториях (подтвердил полную обратную совместимость с Flutter
`ParseService._cacheSet`, которая полностью игнорирует статус/тело ответа);
отдельный subagent (Sonnet) спроектировал и написал
`tests/test_cache_concurrency_regression.py` с barrier-synchronized
concurrency harness и локальным доказательством, что тот же harness ловит
старый баг; финальный независимый скептический review (Opus) подтвердил все
6 пунктов CLOSED, сам прогнал тесты и попытался обойти валидацию ключа —
не нашёл способа. Backend gate: **244/244** (было 229), concurrency-тесты
стабильны на 5 повторных прогонах, `py_compile`/`git diff --check` чистые,
поиск секретов в diff — чисто. Flutter не менялся.

**CR-17 теперь закрыт** — обе подтверждённые проблемы (неатомарность и
...
ключей из PDF (`6 Richtig`, `7a) текст`) с regression-тестом. Smoke выполнялся
на Free-аккаунте Wi-Fi-телефона. Отдельный USB Samsung SM-S938B затем подтвердил
`Premium-Konto`; прежнее расхождение оказалось двумя разными устройствами и
аккаунтами, а не entitlement-багом. Результат виден на экране, но новая попытка
после выхода начинается с нуля — persisted progress не реализован и не должен
считаться закрытым этим Web Creator MVP.

Отдельный Premium Web→Android smoke затем также завершён 21 июля 2026 на USB
Samsung SM-S938B: Premium UID загрузил приватную библиотеку в Creator,
опубликовал безопасный пример, Android после cold restart синхронизировал
четвёртый курс, открыл `Lesen Teil 2 · web-creator` и показал `2/2`. Таким
образом, сам контракт Creator → backend → Android подтверждён для обоих
entitlement; Hosting по-прежнему не развёрнут.

Однополевый импорт Web Creator реализован и проверен 21 июля 2026: полный блок
одного `Lesen Teil 2` локально разделяется на название, текст, вопросы и ответы,
затем валидируется существующим schema-v1 parser и остаётся редактируемым до
публикации. Поддержаны natural 6–7/8–9 и строгие явные секции; неоднозначные и
повреждённые входы не угадываются. Повторный импорт требует подтверждения перед
заменой ручных правок. Gemini, backend API и Android contract не изменены.
Gates Web Creator: 27/27 tests, Chrome 3/3, coverage 82.03%, release build.
