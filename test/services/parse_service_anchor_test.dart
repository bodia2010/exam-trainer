...
void main() {
  ...
  group('correctedStartLineForTest', () {
    test('correct start_line with matching anchor is returned unchanged', () {
      ...
      final result = svc.correctedStartLineForTest(
          5, 'Hören Teil 4 (вариант №5) – 100%', docLines);
      expect(result, 5);
    });
    ...
    test(
      ...
      final result =
          svc.correctedStartLineForTest(wrongStartLine, anchor, docLines);
    ...
    test('anchor search works forward too, not just backward', () {
      final docLines = buildDocLines();
      final result =
          svc.correctedStartLineForTest(2, 'Nummer 36  Some Caller', docLines);
      expect(result, 15);
    });

    test('no anchor match anywhere nearby falls back to the original number',
        () {
      final docLines = buildDocLines();
      final result = svc.correctedStartLineForTest(
          7, 'this text appears nowhere in the document', docLines);
      expect(result, 7);
    });
    ...
    test(
      ...
      final result = svc.correctedStartLineForTest(
          515, 'Hören Teil 4 (вариант №5) – 100%', docLines);
      expect(result, 9515);
    });
    ...
    test('nearest match wins when the anchor text coincidentally repeats',
      ...
      final result =
          svc.correctedStartLineForTest(60, 'Nummer 36  repeated text', docLines);
      expect(result, 50);
    });

    test('collapses whitespace noise so a cosmetically-off anchor still matches',
        () {
      final docLines = buildDocLines();
  ...
  group('correctedItemsForTest — de-duplication when the ANCHOR is the wrong field', () {
    test(
      ...
      final docLines = List<String>.generate(200, (i) => 'filler line $i');
      docLines[100] = 'Lesen Teil 3 (вариант №4) – 100%'; // entry A's real home
      docLines[150] = 'Lesen Teil 3 (вариант №5)   -  100%'; // entry B's real home
      final raw = [
        {
          ...
          'anchor': 'Lesen Teil 3 (вариант №5)   -  100%', // hallucinated — names B
        },
        {
          'section_type': 'lesen_teil3',
          'variant_number': 5,
      ...
      final byVariant = {for (final it in items) it.variantNumber: it.startLine};
      expect(byVariant[4], 100,
          reason: 'entry A must keep its own correct start_line, not B\'s');
      expect(byVariant[5], 150);
    });

    test('a genuine digit-drop still gets corrected when there is no conflict',
        () {
      final docLines = List<String>.generate(10000, (i) => 'filler line $i');
      docLines[9515] = 'Hören Teil 4 (вариант №5) – 100%';
      final raw = [
