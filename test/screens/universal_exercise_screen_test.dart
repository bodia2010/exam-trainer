...
void main() {
  group('buildContentSpan', () {
    test('plain text with no headings becomes a single span', () {
      ...
      expect((span.children!.single as TextSpan).style!.fontWeight,
          isNot(FontWeight.w700));
    });

    test('a single heading in the middle produces three spans: body, bold, body',
        () {
      final span =
          buildContentSpan('Vorher. **TOP 1 Begrüßung** Nachher.');
    ...
    test('a heading at the very start has no leading body span', () {
      final span = buildContentSpan('**Zugangskontrolle und Zeiterfassung**\nRest des Textes.');
      final children = span.children!.cast<TextSpan>();
    ...
    test('multiple headings (a Protokoll with several TOP items) all render bold',
        () {
      final span = buildContentSpan(
          '**TOP 1 Begrüßung**\nErster Absatz.\n**TOP 2 Probleme**\nZweiter Absatz.');
      final children = span.children!.cast<TextSpan>();
      final headings = children.where((c) => c.style!.fontWeight == FontWeight.w700);
      expect(headings.map((c) => c.text).toList(),
          ['TOP 1 Begrüßung', 'TOP 2 Probleme']);
    });

    test('does not strip the asterisks from the plain-text output — they are only in the source markup',
        () {
